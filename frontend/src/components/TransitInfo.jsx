...
const TransitInfo = ({ stops, setLiveVehicleMarkers }) => {
    ...
    useEffect(() => {
        ...
        const fetchVehiclePositions = async (stop) => {
            ...
            ? `/bart-positions/by-stop?stopCode=${stopCode}&agency=bart`
        ...
        const handleStopClick = useCallback(async (stop) => {
            ...
            const { stopCode, agency } = normalizeStopInfo(stop);
            console.log("[handleStopClick] selected stop:", { stopCode, agency });
            ...
            if (cached) {
                console.log("[handleStopClick] using cached data for", stopId);
            ...
            console.log("[handleStopClick] fetching from:", predictionURL);
            ...
            try {
                ...
                console.warn("[handleStopClick] fetch error:", err.message);
        ...
        const handleRefreshSchedule = useCallback(async () => {
            ...
            console.log("[handleRefreshSchedule] calling:", endpoint);
            ...
            try {
                ...
                console.warn('[handleRefreshSchedule] error:', err.message);
                setError('Failed to refresh schedule.');
            } finally {
                setLoading(false);
            }
        }, [selectedStopId, setCachedSchedule, stopsArray]);
        ...
        const renderRouteInfo = (route) => (
            ...
            {route.vehicle?.lat && route.vehicle?.lon ? (
                ...
                <Typography variant="caption" color="text.secondary">
                Nearest Stop: {nearestStops[`${route.vehicle.lat},${route.vehicle.lon}`] || 'Loading...'}
                </Typography>
                </>
            ) : (
